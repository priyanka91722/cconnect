...
3. Set your laptop LAN IP for mobile testing:
```bash
# PowerShell example
$env:EXPO_PUBLIC_API_HOST="YOUR_LOCAL_IP"
```
Or update `mobile/app.json`:
```json
"extra": {
  "apiHost": "YOUR_LOCAL_IP",
  "apiPort": 5000
}
```
> Use your computer's local IP address, not `localhost` or `127.0.0.1`, for physical phones.
