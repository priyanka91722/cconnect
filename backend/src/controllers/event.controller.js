...
const { sendNotificationsToUsers } = require("../utils/notifications");
...
const getCalendarEvents = asyncHandler(async (req, res) => {
  ...
  const query = {
    is_deleted: false,
    is_cancelled: false,
    $or: [{ department: userDepartment }, { department: "All" }],
  };
...
const createEvent = asyncHandler(async (req, res) => {
  ...
  sendNotificationsToUsers({
    ...
    body: `${event.title} is scheduled for ${new Date(event.date).toLocaleString()}.`,
    ...
    console.error("Notification error for new event:", error);
  });
...
const checkInToEvent = asyncHandler(async (req, res) => {
  ...
  if (now < oneHourBefore) {
    throw new AppError(
      "Check-in is not yet available for this event",
