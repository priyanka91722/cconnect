...
const createPost = asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    ...
    postData.attachment_type = req.file.mimetype.startsWith("image/")
      ? "image"
      : "pdf";
  }
  ...
  // Send notifications for all new posts
  sendNotificationsToUsers({
    title: "New Announcement",
    body: post.description ? `${post.title}: ${post.description}` : post.title,
    link: `/posts/${post._id}`,
    type: post.is_event ? "event_post" : "post",
    department: post.department,
    excludedUserIds: [req.user._id],
    created_by: req.user._id,
  }).catch((error) => {
    console.error("Notification error for new post:", error);
  });

  // Send an additional urgent notification if needed
  if (post.is_urgent) {
    sendBulkNotifications({
      ...
      console.error("Urgent notification error for post:", error);
    });
  }
...
const updatePost = asyncHandler(async (req, res) => {
  ...
  // Check ownership
  if (
    post.created_by.toString() !== req.user._id.toString() &&
    req.user.role !== "admin"
  ) {
    throw new AppError(
  ...
  if (req.file) {
    ...
    updates.attachment_type = req.file.mimetype.startsWith("image/")
      ? "image"
      : "pdf";
  }
