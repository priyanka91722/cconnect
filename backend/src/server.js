...
const backfillPostEvents = async () => {
  try {
    ...
    console.log(
      `Backfilling ${postsNeedingEvents.length} post(s) with linked events...`,
    );
    for (const post of postsNeedingEvents) {
      try {
        ...
        console.error(
          `Failed to backfill event for post ${post._id}:`,
          err.message,
        );
      }
    }
...
// Simple root route (useful for network checks)
app.get("/", (req, res) => {
...
connectDB().then(() => {
  app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(
      `Server running (${envMode}) on port ${PORT}\n` +
        `LAN URL: http://${localIP}:${PORT}/`,
    );
  });
});
